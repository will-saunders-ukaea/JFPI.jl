...
def test_gaussian_eval_1():
    ...
    for ix in range(A.npart_local):
        assert abs(PPMD.getindex(A, "Q").data[ix,0] - correct.dat.data_ro[ix]) < 1E-14
...
def test_gaussian_eval_vector_1():
    ...
    VV = VectorFunctionSpace(mesh, "DQ", p, 2)
    ...
    correct = interpolate(
        as_vector(
            (
                (2.0 / sqrt(pi)) * exp(-(2.0 * ((x - 0.5)**2 + (y - 0.5)**2))),
                (2.0 / sqrt(pi)) * exp(-(2.0 * ((x - 0.8)**2 + (y - 0.8)**2))),
            )
        ),
        VV
    )
    ...
    for ix in range(A.npart_local):
        assert np.linalg.norm(PPMD.getindex(A, "Q").data[ix,:] - correct.dat.data_ro[ix], np.inf) < 1E-14
    ...
    for ix in range(A.npart_local):
        ...
        val = Q.data[ix, :]
        cor = correct.at(*pos)
        err = np.linalg.norm(val - cor, np.inf)
        assert err < 1E-10
